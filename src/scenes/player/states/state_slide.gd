...
func on_enabled(previous_state: PlayerState, data: Dictionary = {}):
	...
	var direction: int = sign(player.velocity.x) if player.velocity.x != 0 else player.get_pad_x()
...
func process(delta):
	...
	if not player.crouching:
		if player.is_action_pressed("run") or abs(player.velocity.x) > player.get_state_data(Player.STATE.WALK)["MAX_SPEED"]:
			player.change_state(Player.STATE.RUN)
		elif player.get_pad_x() != 0:
