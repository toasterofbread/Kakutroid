...
func on_enabled(previous_state: PlayerState, data: Dictionary = {}):
	...
	if "dash_magnitude" in data and data["dash_magnitude"] != 0.0 and direction != 0:
		player.vel_move_x(physics_data["DASH_SPEED"] * direction * data["dash_magnitude"])
...
func process(delta):
	...
	if Input.is_action_just_pressed("jump"):
		var magnitude: float = abs(player.velocity.x) / physics_data["DASH_SPEED"]
	...
	if not player.crouching:
		if Input.is_action_pressed("run") or abs(player.velocity.x) > player.get_state_physics_data(Enums.PLAYER_STATE.WALK)["MAX_SPEED"]:
