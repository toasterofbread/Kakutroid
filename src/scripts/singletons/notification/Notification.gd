...
onready var types = {
	"text": preload("res://src/scenes/ui/notifications/TextNotification.tscn"),
	"largetext": preload("res://src/scenes/ui/notifications/LargeTextNotification.tscn"),
	"buttonprompt": preload("res://src/scenes/ui/notifications/ButtonPromptNotification.tscn"),
}
const lengths = {
	"long": 5.0,
	"normal": 3.0,
...
func add(node: Control, clear):
	...
	if clear is float:
		yield(get_tree().create_timer(clear), "timeout")
		clear_single(node)
	elif clear is Array:
